...
db = SQLAlchemy(app)
api = Api(app, version='1.0', title='API Doc', description='COMP9321 ASS2', doc='/')
...
class CreateEventHandler(Resource):
    ...
    def get(self):
        ...
        if ('+id' in orders and '-id' in orders) or ('+name' in orders and '-name' in orders) or (
                '+datetime' in orders and '-datetime' in orders):
            return {
                'message': f'Invalid order: cannot order a field ascendingly and descendingly at the mean time.'}, 400
        ...
        for order in orders:
            ...
            if field != 'datetime':
                ...
                order_list.append(
                    eval("Event.date.concat(' ').concat(Event.from).asc()") if sign == '+' else eval(
                        "Event.date.concat(' ').concat(Event.from).desc()"))

        # Retrieve events from database
        events = Event.query.order_by(*order_list).with_entities(*field_list).paginate(page=page, per_page=size,
...
class EventByIDHandler(Resource):
    ...
    def get(self, id):
        ...
        # Links
        links = OrderedDict()
        links["self"] = {"self": {"href": "/events/{0}".format(event.id)}}
        if previous_event:
...
class WeatherHandler(Resource):
    ...
    def get(self):
        ...
        cities = ['Sydney', 'Darwin', 'Brisbane', 'Perth', 'Adelaide', 'Cairns', 'Melbourne', 'Canberra', 'Hobart']
        cities_weather = city_dataset[city_dataset['city'].isin(cities)].loc[:, ['city', 'lat', 'lng']]
        cities_weather['coordinates'] = 'POINT (' + cities_weather['lng'].astype(str) + ' ' + cities_weather[
            'lat'].astype(str) + ')'
        ...
        for city in cities:
            ...
            ax.annotate(city + '\n' + weather['weather'] + ' ' + weather['temperature'], xy=(lng, lat),
                        xytext=(lng - 1, lat + 1), color='blue')
...
class EventResponse:
    def __init__(self, event):
        ...
        self.response["_links"] = {"self": {"href": "/events/{0}".format(event.id)}}
...
def get_weather_forecast(lat, lon, timedelta):
    ...
    wind_speed = wind_speed_mapping.get(wind_speed, "Undefined")
    ...
    humidity = "Undefined" if humidity == -9999 else humidity

    temperature = forecast['temp2m']
    temperature = "Undefined" if temperature == -9999 else f"{temperature} C"
